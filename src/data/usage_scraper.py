"""
Scrapes pokedb.tokyo for Pokemon Champions usage data.
...
from src.constants import ABILITIES_JA, NATURES_JA
...
USAGE_SOURCE_DEFAULT = "pokedb_tokyo"
...
def _fetch_text(url: str, retries: int = 3, retry_delay: float = 2.0) -> str | None:
    for attempt in range(retries + 1):
        try:
            ...
            response = _SESSION.get(url, timeout=_REQUEST_TIMEOUT, headers=headers)
            ...
            if attempt < retries:
                ...
                _log(f"[ERROR] fetch failed {url}: {e}")
                return None
    return None
...
def _match_known_name(text: str, ordered_names: list[str], _cache: dict = {}) -> str | None:
    cache_key = id(ordered_names)
    cache_entry = _cache.get(cache_key)
    if cache_entry is None:
        valid_names = [name for name in ordered_names if name]
        name_set = set(valid_names)
        # Prefer longer names first so form names win over base names.
    ...
    name_set: set[str] = cache_entry["name_set"]  # type: ignore[index]
    if text in name_set:
        return text
...
def _supplement_ev(ev: dict[str, int], species_id: int) -> dict[str, int]:
    """Fill remaining EV points (up to total 66) by project rules.

    Rules:
    1. Original S == 0 -> add 2 to S.
    2. Original S >= 1 and original H == 0 -> add 2 to H.
    3. Original S >= 1 and original H >= 1 -> add 2 to A or C (higher base stat).
    4. Remaining points: H up to 32.
    5. Remaining points: S up to 32.
    6. Remaining points: B up to 32.
    """
    total = sum(ev.values())
    remainder = 66 - total
    if remainder <= 0:
    ...
    ev = dict(ev)
    original_s = int(ev.get("S", 0))
    original_h = int(ev.get("H", 0))

    def _add_with_cap(stat_key: str, add_amount: int, cap: int = 32) -> int:
        current = int(ev.get(stat_key, 0))
        room = max(0, cap - current)
        delta = max(0, min(add_amount, room))
        ev[stat_key] = current + delta
        return delta

    # Initial +2 allocation based on original values
    if original_s == 0 and remainder > 0:
        remainder -= _add_with_cap("S", 2)
    elif original_s >= 1 and original_h == 0 and remainder > 0:
        remainder -= _add_with_cap("H", 2)
    elif original_s >= 1 and original_h >= 1 and remainder > 0:
        species = db.get_species_by_id(species_id)
        target = "C" if (species and species.base_sp_attack > species.base_attack) else "A"
        remainder -= _add_with_cap(target, 2)

    # Remaining allocation
    if remainder > 0:
        remainder -= _add_with_cap("H", remainder)
    if remainder > 0:
        remainder -= _add_with_cap("S", remainder)
    if remainder > 0:
        remainder -= _add_with_cap("B", remainder)
...
def _scrape_usage_pokedb_tokyo(
    ...
    requested_season = db.normalize_season_token(season)
    _log("=" * 60)
    _log(f"Starting pokedb.tokyo usage data scraping for season: {requested_season}")
    _log("=" * 60)
    ...
    # Omit season so pokedb.tokyo serves the latest available season.
    list_url = f"{POKEDB_TOKYO_BASE}/pokemon/list?rule=0"
    ...
    season_token = _extract_pokedb_season(list_html, requested_season)
    _log(f"Resolved pokedb.tokyo season: {season_token}")
...
def _extract_pokedb_season(page_html: str, fallback: str) -> str:
    """Extract the displayed Champions season token from a pokedb.tokyo page."""
    patterns = (
        r"<title\b[^>]*>.*?(M-\d+).*?</title>",
        r"<meta\b[^>]+(?:name|property)=[\"'](?:description|og:title|twitter:title)[\"'][^>]+content=[\"'][^\"']*(M-\d+)",
        r"<option\b[^>]*\bselected\b[^>]*>.*?(M-\d+).*?</option>",
    )
    for pattern in patterns:
        match = re.search(pattern, page_html, re.IGNORECASE | re.DOTALL)
        if match:
            return db.normalize_season_token(match.group(1))
    return db.normalize_season_token(fallback)


def _extract_ev_pokedb(stat_col: object) -> dict[str, int]:
    """Extract best EV spread from pokedb trend stats.

    Priority:
    1. Aggregated top label (e.g. "HA 49.7%") -> map letters to 32 points each.
    2. Concrete "individual" spreads (e.g. H32A32S2) by highest percentage.
    3. Aggregate-like numeric rows as final fallback.
    """
    # 1) Aggregated label mode: "1HA49.7%..." means H32/A32 base allocation.
    agg_best_rate = -1.0
    agg_best_letters = ""
    for li in stat_col.find_all("li"):
        text = li.get_text("", strip=True)
        m = re.match(r"^\d+([HABCDS]{1,6})(?:[+a-z]*)?(\d+(?:\.\d+)?)%", text)
        if not m:
            continue
        letters = m.group(1)
        rate = float(m.group(2))
        if rate > agg_best_rate:
            agg_best_rate = rate
            agg_best_letters = letters
    if agg_best_letters:
        return {k: 32 for k in agg_best_letters}

    # 2) and 3) Numeric extraction fallback.
    candidates: list[tuple[float, dict[str, int], bool]] = []
    ...
    for li in stat_col.find_all("li"):
        ...
        # Keep percentage for ranking candidates (higher usage first).
        rate_match = re.search(r"(\d+(?:\.\d+)?)%", text)
        rate = float(rate_match.group(1)) if rate_match else 0.0

        # Extract HABCDS points anywhere in the list-item text.
        ev = {k: int(v) for k, v in re.findall(r"([HABCDS])(\d{1,2})", text)}
        if not ev:
            continue

        # Aggregate rows often include "31+" or "". Treat them as lower-priority.
        is_aggregate_like = ("余り" in text) or bool(re.search(r"[HABCDS]\d+\+", text))
        total = sum(ev.values())
        is_valid_total = total <= 66
        if not is_valid_total:
            continue

        candidates.append((rate, ev, is_aggregate_like))

    if not candidates:
        return {}

    concrete = [row for row in candidates if not row[2]]
    if concrete:
        concrete.sort(key=lambda row: row[0], reverse=True)
        return concrete[0][1]
...
class UsageScraper(QThread):
    ...
    def run(self) -> None:
        ...
        if snapshot:
            snapshot_season = db.normalize_season_token(snapshot.get("season", self.season))
            self.season = snapshot_season
            db.set_active_usage_season(snapshot_season)
            ...
            db.export_usage_to_json(snapshot_season)
            self.finished.emit(
                True,
